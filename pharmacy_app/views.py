...
def create_prescription(request, encounter_id):
    ...
    if request.method == "POST":
        ...
        if formset.is_valid():
            ...
            messages.success(
                request, f"Prescription {prescription.id} created securely."
            )
            return redirect(
                "pharmacy_app:prescription_detail",
...
def prescription_detail(request, prescription_id):
    prescription = get_object_or_404(
        Prescription.objects.select_related(
            "encounter__patient", "encounter__staff", "dispensedBy"
        ).prefetch_related("items"),
        id=prescription_id,
    )
    ...
    return render(
        request,
        "pharmacy_app/prescription_detail.html",
        {"prescription": prescription, "active_nav": "prescriptions"},
    )
...
def validate_prescription(request, prescription_id):
    ...
    return render(
        request,
        "pharmacy_app/validate_prescription.html",
        {"prescription": prescription, "active_nav": "prescriptions"},
    )
...
def dispense_medicine(request, prescription_id):
    ...
    if request.method == "POST":
        ...
        with transaction.atomic():
            ...
            prescription.save(
                update_fields=["status", "dispensedAt", "dispensedBy"]
            )
    ...
    return render(
        request,
        "pharmacy_app/dispense_medicine.html",
        {"prescription": prescription, "active_nav": "prescriptions"},
    )
